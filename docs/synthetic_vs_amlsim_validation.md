...
To test whether our architecture benefits from more data and to verify reproducibility, we re-ran the AMLSim validation at ~10× the initial node count under strict determinism constraints (including seed locking, node sorting, and component-split stabilization).

We ran the pipeline twice independently and verified split hashes and metrics matched exactly to 6 decimal places.
...
| **Total edges** | 41,400 | 70,560 |
| **Positive (laundering)** | 6,357 (20.0%) | 6,357 (11.3%) |
| **Negative (clean)** | 25,428 (80.0%) | 50,000 (88.7%) |
| **Test positives** | 3,816 | 642 |
| **Test negatives** | 5,086 | 10,000 |
| **Split Hash (Reproducible)**| — | `a39d22d05ef8c5d6efde92ad6577b68c9193304ede5871eb9373c9c6f90b37ce` |
...
> "We validated our GraphSAGE architecture against the IBM AMLSim benchmark — a public AML research dataset with 5 million transactions. At 56,000-node scale, our model achieved **85.5% AUC** with zero domain-specific tuning, improving from 70.6% AUC at smaller scale. Under a 35% edge dropout scenario, the model remains robust with **84.1% AUC**, confirming it generalizes reliably to unobserved transactional links."
