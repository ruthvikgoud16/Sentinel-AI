...
# ============================================================
print("=" * 60)
print("STEP 2: Subsampling AMLSim HI-Small dataset")
print("=" * 60)
...
for chunk in pd.read_csv('data/amlsim/HI-Small_Trans.csv', chunksize=200000):
    ...
    for _, row in launder_rows.iterrows():
        from_id = f"{row['From Bank']}_{row['Account']}"
        to_id = f"{row['To Bank']}_{row['Account.1']}"
        laundering_accounts.add(from_id)
        laundering_accounts.add(to_id)
...
# Pass 2: Sample background accounts and their transactions
# With 6,357 laundering accounts already exceeding the 5-8K target,
# we need substantial background to avoid an inverted class balance.
# Target ~20% positive rate: need ~4x laundering count in background.
target_background = max(len(laundering_accounts) * 4, 25000)

print(f"Pass 2: Sampling ~{target_background} background accounts (chunked)...")
background_accounts = set()
background_edges = []

# Collect a pool of candidate background accounts
bg_candidates = set()
for chunk in pd.read_csv('data/amlsim/HI-Small_Trans.csv', chunksize=200000):
    ...
    for _, row in clean_rows.iterrows():
        from_id = f"{row['From Bank']}_{row['Account']}"
        to_id = f"{row['To Bank']}_{row['Account.1']}"
        if from_id not in laundering_accounts:
            bg_candidates.add(from_id)
        if to_id not in laundering_accounts:
            bg_candidates.add(to_id)
    if len(bg_candidates) >= target_background * 2:
        break  # Enough candidates collected
...
# Pass 3: Collect ALL edges between selected accounts
print("Pass 3: Collecting edges between selected accounts (chunked)...")
all_edges = list(laundering_edges)  # Start with laundering edges
...
for chunk in pd.read_csv('data/amlsim/HI-Small_Trans.csv', chunksize=200000):
    for _, row in chunk.iterrows():
        from_id = f"{row['From Bank']}_{row['Account']}"
        to_id = f"{row['To Bank']}_{row['Account.1']}"
        if from_id in all_selected_accounts and to_id in all_selected_accounts:
            pair = (from_id, to_id)
...
with open('results/model_metrics_amlsim.json', 'w') as f:
    json.dump(metrics, f, indent=4)
print(f"\nMetrics saved to results/model_metrics_amlsim.json")
print("Done.")
